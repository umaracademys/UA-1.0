...
import { User, Mail, Phone, Calendar, Edit, Users } from "lucide-react";
...
export default function TeacherProfilePage() {
  ...
  const fetchTeacherData = async () => {
    try {
      ...
      const response = await axios.get(`/api/teachers/${user?.teacher?._id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setTeacherData(response.data.teacher || response.data);
    } catch (error) {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        {teacherData && (
          <div className="rounded-lg border border-neutral-200 bg-white p-6">
            <h2 className="mb-4 text-lg font-semibold text-neutral-900">Teacher Information</h2>
            <div className="space-y-4">
              {teacherData.specialization && (
                <div>
                  <p className="text-xs text-neutral-500">Specialization</p>
                  <p className="text-sm font-medium text-neutral-900">{teacherData.specialization}</p>
                </div>
              )}
              {teacherData.joinDate && (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs text-neutral-500">Join Date</p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-neutral-500">Assigned Students</p>
                  <p className="text-sm font-medium text-neutral-900">
                    {teacherData.assignedStudents?.length || 0} student(s)
                  </p>
                </div>
              </div>
              <div>
                ...
                <p
                  className={`text-sm font-medium ${
                    teacherData.status === "active" ? "text-green-600" : "text-yellow-600"
                  }`}
                >

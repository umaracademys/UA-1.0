 ...
 * Teacher resumes a paused listening session. Timer continues; range stays locked.
 */
  ...
  try {
    ...
    if (ticket.status !== "paused") {
      return NextResponse.json(
        { success: false, message: "Only a paused ticket can be resumed." },
        { status: 400 },
      );
    }

    if (ticket.teacherId?.toString() !== teacher._id.toString()) {
      return NextResponse.json({ success: false, message: "Only the assigned teacher can resume." }, { status: 403 });
    }
    ...
    return NextResponse.json(
      { success: false, message: (error as Error).message || "Resume failed." },
      { status: 500 },
    );
  }
}
